...
class BuyScoringEngine:
    ...
    def _fundamental_score(
        ...
        # Safe extraction: Handing None values from APIs directly
        ...
        pb = float(pb) if pb is not None else 999.0

        peg = fundamentals.get("peg")
        peg = float(peg) if peg is not None else 999.0

        debt = fundamentals.get("debt_to_equity")
        debt = float(debt) if debt is not None else 999.0
